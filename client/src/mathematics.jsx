...
function Mathematics({ onBackHome, onBackCourses, courseLabel = "11 JEE-Advance", courseQuery = "11 JEE-Advance" }) {
  const [notes, setNotes] = useState([]);
  const [error, setError] = useState("");

  useEffect(() => {
    fetch(`${API_BASE}/api/notes?course=${encodeURIComponent(courseQuery)}&subject=Mathematics`)
  ...
  return (
    <div className="course-page">
      ...
      <div className="course-shell">
        <h1 className="course-title">{courseLabel} - Mathematics</h1>
        {error && <p className="form-error">{error}</p>}
        {notes.length === 0 && !error && <p>No notes uploaded yet.</p>}
        {notes.length > 0 && (
          <div className="course-notes">
            <h2 className="course-notes-title">Mathematics Notes</h2>
